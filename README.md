...
Windows에서 Azure Kinect Developer Kit의 depth camera 값을 `pyk4a`로 획득하고, 실시간 preview, colorbar range 검증, uint8 PNG capture 저장을 수행하는 최소 검증 프로젝트입니다.

주요 기준:

- Azure Kinect Sensor SDK: https://github.com/microsoft/Azure-Kinect-Sensor-SDK
- pyk4a PyPI: https://pypi.org/project/pyk4a/
- pyk4a GitHub: https://github.com/etiennedub/pyk4a
...
Anaconda를 사용한다면 Anaconda Prompt에서 실행하는 편이 DLL 탐색 문제가 적습니다.
...
- fps: `FPS_30`
- depth image source: `capture.depth`
...
`Min mm`, `Max mm` 입력칸에서 Enter를 눌러도 range가 적용됩니다. 직접 입력 range를 적용하면 manual mode로 전환됩니다.
...
- 새 터미널에서 다시 실행하세요.
- Anaconda 환경에서는 `CONDA_DLL_SEARCH_MODIFICATION_ENABLE=1`이 필요할 수 있습니다.
...
### `python` 명령이 실행되지 않는 경우

- Windows App execution aliases의 `python.exe`, `python3.exe` 별칭을 끄세요.
- 실제 설치된 Python 경로를 직접 사용하세요.
- Anaconda를 사용한다면 Anaconda Prompt에서 `python --version`이 정상 출력되는지 확인하세요.
